...
const PageActionButtons = ({
  ...
  const addButtonStyle =
    hasUploadButton && hasExportButton ? 'btn btn-primary me-3' : 'btn btn-light-primary me-3'
  return (
...
const ModalFooterButtons = ({onCancel, onSubmit}: any) => {
  return (
    <>
      ...
      ,
      <Button key='submit' type='primary' htmlType='submit' onClick={onSubmit} size='large'>
        Submit
      </Button>
      ,
    </>
...
function calculateVolumesByField(groupedByField: any) {
  ...
  for (const field in groupedByField) {
    ...
    const sumLoads = loads.reduce(
      (accumulator: any, currentValue: any) => accumulator + currentValue
    )
    const sumNominalWeights = nominalWeights.reduce(
      (accumulator: any, currentValue: any) => accumulator + currentValue
    )
    const sumPayloadWeights = payloadWeights.reduce(
      (accumulator: any, currentValue: any) => accumulator + currentValue
    )
...
export {
  ...
  roundOff,
  timeStamp,
  calculateVolumesByField,
  extractDateFromTimestamp,
  batchVolumesThirtyDaysRolling,
}
